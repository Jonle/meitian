...
{
    if (!_arrowView) {
        UIImage *image = [UIImage imageNamed:MJRefreshSrcName(@"arrowNew.png")];
        if (!image) {
            image = [UIImage imageNamed:MJRefreshFrameworkSrcName(@"arrowNew.png")];
        }
    ...
    if (state == MJRefreshStateNormal) {
        if (oldState == MJRefreshStateRefreshing) {
            ...
            [UIView animateWithDuration:MJRefreshSlowAnimationDuration animations:^{
                ...
                if (self.state != MJRefreshStateNormal) return;
